...
#import "PromoAlertView.h"
#import "BlockBackground.h"
#import "BlockUI.h"

#define kPromoAlertViewBorder 10
#define kPromoAlertViewTitleFont [UIFont boldSystemFontOfSize:20]
#define kPromoAlertViewMessageFont [UIFont systemFontOfSize:15]

@implementation PromoAlertView
...
static UIImage *promoimage = nil;

#define PROMOIMAGEWIDTH 260
#define PROMOIMAGESIZE 161
#define PROMOPADDING 10

#define kPromoAlertButtonHeight 38

#pragma mark - init

+ (void)initialize
{
    if (self == [PromoAlertView class])
    {
        ...
        titleFont = [kPromoAlertViewTitleFont retain];
        messageFont = [kPromoAlertViewMessageFont retain];
        buttonFont = [kAlertViewButtonFont retain];
        promoimage = [[UIImage imageNamed: @"Promo.png"] retain];
    }
}

+ (PromoAlertView *)alertWithTitle:(NSString *)title message:(NSString *)message
{
    return [[[PromoAlertView alloc] initWithTitle:title message:message] autorelease];
}
...
- (id)initWithTitle:(NSString *)title message:(NSString *)message 
{
    if ((self = [super init]))
    {
        ...
        _height = kPromoAlertViewBorder + 6;

        if (title)
        {
            CGSize size = [title sizeWithFont:titleFont
                            constrainedToSize:CGSizeMake(frame.size.width-kPromoAlertViewBorder*2, 1000)
                                lineBreakMode:NSLineBreakByWordWrapping];

            UILabel *labelView = [[UILabel alloc] initWithFrame:CGRectMake(kPromoAlertViewBorder, _height, frame.size.width-kPromoAlertViewBorder*2, size.height)];
            ...
            _height += size.height + kPromoAlertViewBorder;
        }
        
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(kPromoAlertViewBorder, _height + PROMOPADDING / 2, PROMOIMAGEWIDTH, PROMOIMAGESIZE)];
        UIImage *bkgImg = [UIImage imageNamed: @"Promo.png"];
        [imageView setImage:bkgImg];
        [_view addSubview:imageView];
        
        _height += PROMOPADDING / 2 + PROMOIMAGESIZE + PROMOPADDING / 2;
...
- (void)show
{
    ...
    for (NSUInteger i = 0; i < _blocks.count; i++)
    {
        ...
        CGFloat maxHalfWidth = floorf((_view.bounds.size.width-kPromoAlertViewBorder*3)*0.5);
        CGFloat width = _view.bounds.size.width-kPromoAlertViewBorder*2;
        CGFloat xOffset = kPromoAlertViewBorder;
        if (isSecondButton)
        {
            width = maxHalfWidth;
            xOffset = width + kPromoAlertViewBorder * 2;
            isSecondButton = NO;
        }
        else if (i + 1 < _blocks.count)
        {
            ...
            CGSize size = [title sizeWithFont:buttonFont 
                                  minFontSize:10 
                               actualFontSize:nil
                                     forWidth:_view.bounds.size.width-kPromoAlertViewBorder*2 
                                lineBreakMode:NSLineBreakByClipping];
            
            if (size.width < maxHalfWidth - kPromoAlertViewBorder)
            {
        ...
        else if (_blocks.count  == 1)
        {
            ...
            if (size.width + 2 * kPromoAlertViewBorder < width)
            {
                width = size.width + 2 * kPromoAlertViewBorder;
                xOffset = floorf((_view.bounds.size.width - width) * 0.5);
            }
        }
        
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        button.frame = CGRectMake(xOffset, _height, width, kPromoAlertButtonHeight);
        ...
        if (!isSecondButton)
            _height += kPromoAlertButtonHeight + kPromoAlertViewBorder;
